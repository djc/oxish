...
impl Channels {
    ...
    pub(crate) fn agent_connection(
        ...
        // The remote ID, send window and maximum packet size come from the
        // client's confirmation of our channel open message.
        entry.insert(Channel {
            remote_id: 0,
            send_window: 0,
            receive_window: INITIAL_WINDOW_SIZE,
    ...
    pub(crate) fn confirmation(
        ...
        channel.send_window = confirmation.initial_window_size;
            ...
            match &mut channel.kind {
                ...
                ChannelKind::Agent(state) => match state {
                    ...
                    AgentState::Open(stream) => {
                        let mut buf = [0u8; 4096];
                        let limit =
                            Ord::min(channel.maximum_packet_size, channel.send_window) as usize;
                        let writable = match limit {
                            0 => continue,
                            _ if limit < buf.len() => &mut buf[..limit],
                            _ => &mut buf[..],
                        ...
                        let mut read_buf = ReadBuf::new(writable);
                        match Pin::new(stream).poll_read(cx, &mut read_buf) {
                            ...
                            Poll::Ready(result) => {
                                ...
                                let data = Cow::Owned(filled.to_vec());
                                channel.send_window =
                                    channel.send_window.saturating_sub(filled.len() as u32);
                                return Poll::Ready(Ok(Some(ChannelEvent::Outgoing(
                                    OutgoingChannelMessage::Data(ChannelData {
                                        recipient_channel,
                                        data,
