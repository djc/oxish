...
use crate::{Connection, Error, platform, receive};
...
impl DefaultStore {
    ...
    pub fn new(provider: &dyn CryptoProvider) -> Result<Box<dyn UserStore>, Error> {
        Ok(match platform::current_user()? {
            None => {
                debug!("using system user store");
                Box::new(SystemStore) as Box<dyn UserStore>
            }
            Some(data) => {
                debug!(user = %data.name, "using single-user store");
                let keys = platform::authorized_keys(&data, provider);
...
impl UserStore for SystemStore {
    fn lookup(&self, name: Username) -> Option<User> {
        match platform::lookup_user(name) {
            Ok(user) => Some(user),
            Err(error) => {
    ...
    fn keys(&self, user: &User, provider: &dyn CryptoProvider) -> Vec<AuthorizedKey> {
        platform::authorized_keys(user, provider)
    }
